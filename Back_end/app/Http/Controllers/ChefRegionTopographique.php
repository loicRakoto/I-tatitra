...
class ChefRegionTopographique extends Controller
{
    public function ListeActiviteRegion(Request $request)
    {
        ...
        $recupNomRegion = DB::table('circonscriptions')->where('id', $circonscriptionId)->first();
        $NomRegion = $recupNomRegion->NomRegion;
        ...
        foreach ($idCircons as $idCircon) {
            ...
            for ($activiteId = 1; $activiteId <= 11; $activiteId++) {
                ...
                $LastEnregistrement = DB::table('enregistrers')
                    ...
                    ->whereMonth('enregistrers.created_at', $mois)
    ...
    public function RechercheRapportActiviteRegionale(Request $request)
    {
        $circonscriptionId = $request->utilisateurCirconscriptionId;
    ...
    public function ListeDateActiviteRegionale(Request $request)
    {
        ...
        $ListEnregistrement = DB::table('enregistrers')
            ...
            ->orderBy('annee', 'desc')
            ->orderBy('mois', 'desc')
            ->where('circonscriptions.NomRegion', '=', $NomRegion)
            ->get();
